...
从 v0.2.8 开始，LiveHime 会自己检查 GitHub Releases 上的新版本（“直播姬”面板 → 更新）：

- **自动（默认）：** 启动后和每隔几个小时检查一次，在后台下载，退出 LiveHime 时安装；也可以点“立即重启并更新”。
- **手动：** 关掉“自动检查并下载更新”后，点“检查更新”→“下载并安装”，走同一个下载和校验流程。
- **测试版：** 打开“参与测试版”会提前收到标记为测试版的新版本。
- **跳过 / 回退：** 可以跳过某个版本；更新后可以“回退到上一版本”。每次安装前都会把场景和设置备份到
  `~/Library/Application Support/LiveHime/backups`（保留最近 8 份）。
- 安装前会核对下载文件的校验值，并确认新版本和当前应用由同一张证书签名，所以登录、设置和屏幕录制等权限都会保留。
  直播或录制时不会重启。

从 v0.2.7 或更早的版本升级到 v0.2.8，需要按下面的方法手动替换一次。
